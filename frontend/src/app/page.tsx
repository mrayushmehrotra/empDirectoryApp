...
export default function Page() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-purple-100 py-10 px-4">
      <div className="max-w-4xl mx-auto bg-white text-black rounded-xl shadow-lg p-8">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
          <h1 className="text-3xl font-bold text-blue-700">Employees</h1>
          <div className="flex gap-2 items-center">
            <select
              className="border   text-black  border-blue-200 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400"
              value={selectedDept}
            ...
            <Link href="/employee/new">
              <button className="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-4 py-2 rounded shadow transition">Add New Employee</button>
            </Link>
          </div>
        </div>
            ...
            <tbody>
              {employees.map((emp: any) => (
                <tr key={emp.id} className="border-t border-blue-50 hover:bg-blue-50 transition">
                  <td className="py-2 px-4 font-semibold text-blue-900">{emp.name}</td>
                  <td className="py-2 text-black   px-4">{emp.position}</td>
                  <td className="py-2 text-black px-4">{emp.department}</td>
